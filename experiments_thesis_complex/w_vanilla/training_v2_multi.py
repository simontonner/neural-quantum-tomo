...
def run_single_experiment(
    ...
    print(f"Using N={len(dataset)} samples, n={nv} qubits, device={DEVICE}, seed={cfg['seed']}")
...
def plot_overlap_curve(sample_sizes, overlaps, num_qubits: int, seed: int):
    ...
    ax.plot(x_values, y_values, "s-", color="#a24a55", label=rf"RBM - $N = {num_qubits}$ - seed {seed}", zorder=2)
...
if __name__ == "__main__":
    ...
    for seed in seeds:
        ...
        save_seed_results_csv(
            results_dir / f"overlap_{NUM_QUBITS}_seed{seed}.csv",
            seed,
            cfg["sample_sizes"],
            overlaps,
        )
        ...
        plot_overlap_curve(cfg["sample_sizes"], overlaps, num_qubits=NUM_QUBITS, seed=seed)