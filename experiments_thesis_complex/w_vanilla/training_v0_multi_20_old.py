...
from data_handling import load_measurements_txt, load_state_txt
...
def load_vanilla_w_dataset(
    train_path: Path | str,
    psi_path: Path | str,
    n_use: int | None = None,
):
    values_np, bases, _ = load_measurements_txt(Path(train_path))
    amps_np, _ = load_state_txt(Path(psi_path))

    if any(b.upper() != "Z" for b in bases):
        raise ValueError("This script only supports computational-basis Z data.")

    train_samples = torch.tensor(values_np, dtype=DTYPE, device=DEVICE)
    if n_use is not None:
        n_use = min(int(n_use), int(train_samples.shape[0]))
        train_samples = train_samples[:n_use].contiguous()

    target_state = torch.tensor(amps_np, dtype=torch.cdouble, device=DEVICE)
    num_visible = int(train_samples.shape[1])

    if target_state.numel() != (1 << num_visible):
        raise ValueError(
            f"State dimension {target_state.numel()} does not match {num_visible} qubits."
        )

    return train_samples, target_state, num_visible


class SimpleLoader:
    def __init__(self, samples: torch.Tensor, batch_size: int, seed: int = 0):
    ...
    def iter_epoch(self):
        ...
        gen = torch.Generator(device="cpu").manual_seed(self.seed + self.epoch)
        ...
        for start in range(0, N, self.batch_size):
            ...
            idx = order[start:end].to(self.samples.device)
            pos_batch = self.samples[idx]
            yield pos_batch, None, None
        ...
        *,
        ...
        for ep in range(1, epochs + 1):
            for pos_batch, neg_batch, bases_batch in loader.iter_epoch():
                loss, _ = self((pos_batch, neg_batch, bases_batch), {"rng": rng})
...
def w_overlap_positive(model: PositiveWaveFunction, M: int = 10_000, k: int = 100) -> float:
    """
    Same proxy structure as in your original code.
    """
    N = model.num_visible
    rbm = model.rbm
    device = model.device
...
def run_single_experiment(
    ...
    train_samples, target_state, nv = load_vanilla_w_dataset(train_path, psi_path, n_use=N_use)
    nh = nv * cfg["hidden_factor"]

    print(f"Using N={N_use} samples, n={nv} qubits, device={DEVICE}, seed={cfg['seed']}")

    loader = SimpleLoader(train_samples, batch_size=cfg["pos_batch_size"], seed=cfg["seed"])
...
def save_seed_results_csv(csv_path: Path, seed: int, sample_sizes: list[int], overlaps: list[float]):
    csv_path.parent.mkdir(parents=True, exist_ok=True)

    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["timestamp", "seed", "samples", "overlap"])

        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        for samples, overlap in zip(sample_sizes, overlaps):
            writer.writerow([timestamp, seed, samples, overlap])

    print(f"Saved results to {csv_path}")


def plot_overlap_curve(sample_sizes, overlaps, seed):
    ...
    ax.plot(x_values, y_values, "s-", color="#a24a55", label=rf"RBM - $N = 20$ - seed {seed}", zorder=2)
...
if __name__ == "__main__":
    ...
    seeds = [
        1234, 2024, 7, 42, 314,
        2718, 9001, 1337, 5555, 8080,
        1111, 2222, 3333, 4444, 9999,
    ]

    train_path = Path("measurements/w_vanilla_20_meas_50000.txt")
    psi_path = Path("state_vectors/w_vanilla_20_state.txt")
    ...
    for seed in seeds:
        ...
        overlaps_by_seed[seed] = overlaps

        save_seed_results_csv(
            results_dir / f"overlap_20_seed{seed}.csv",
            seed,
            cfg["sample_sizes"],
            overlaps,
        )
        ...
        plot_overlap_curve(cfg["sample_sizes"], overlaps, seed)