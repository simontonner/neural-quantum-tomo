...
def print_phase_rows(rows: list[dict], seed: int, max_rows: int | None = None) -> None:
    print(f"\nPhase rows for seed {seed}:")
    n_show = len(rows) if max_rows is None else min(len(rows), int(max_rows))
...
if __name__ == "__main__":
    ...
    seeds = [1234, 4444, 5555, 1618, 1732, 9793, 8979, 4626, 4338, 3279]
    ...
    for seed in seeds:
        ...
        phase_rows = extract_phase_rows(model, target_state, basis_states)
        all_rows_by_seed[seed] = phase_rows

        save_phase_rows_csv(results_dir / f"w_phase_phases_seed{seed}.csv", phase_rows)
        print_phase_rows(phase_rows, seed=seed, max_rows=None)

        plot_training_curves(history, pcd_k=cfg["pcd_k"], seed=seed)
        plot_phase_comparison_from_rows(phase_rows, seed=seed)
