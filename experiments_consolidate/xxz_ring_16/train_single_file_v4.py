...
class RBM(nn.Module):
    def __init__(self, num_visible: int, num_hidden: int, k: int = 1, T: float = 1.0,
        ...
        # PCD: Persistent buffer for fantasy particles
        # We register it as a buffer so it moves to GPU with the model but isn't a parameter
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        # === PCD LOGIC ===
        # 1. Detach current persistent chain from graph (stop gradients flowing back in time)
        v_model = self.persistent_chain.detach().to(v_data.device)

        # 2. Run Gibbs steps (PCD usually works well with even k=1, we use small k)
        for _ in range(self.k):
            v_model = self._gibbs_step(v_model, self.T, rng)

        # 3. Update persistent buffer
        self.persistent_chain = v_model.clone()
        ...
        # Metrics
        # Check if persistent chain has collapsed (all same) or is mixing
        mag_model = v_model.float().mean()

        # Calculate recent flip rate on the persistent chain
        # (Compare step t with step t+k)
        prev_chain = self.persistent_chain.detach() # approximation for metric
        # Actually better to compare v_model vs v_model_start but let's just use mag for now
...
def train(model: nn.Module, optimizer: torch.optim.Optimizer, loader, num_epochs: int,
    ...
    # PCD requires smaller LR usually
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)

    print(f"{'Epoch':<6} | {'Loss':<9} | {'FE Diff':<9} | {'Mag':<9} | {'LR':<9}")
    print("-" * 55)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in loader:
            ...
            tot_loss += float(loss)
            avg_fe_diff += float(aux_out.get("fe_diff", 0.0))
        ...
        print(f"{epoch + 1:<6} | {tot_loss/n:+.4f}   | {avg_fe_diff/n:+.4f}    | "
              f"{avg_mag/n:.4f}    | {lr:.5f}")
...
def compute_renyi_entropy_from_samples(
    ...
    # Note: RBM energy can be large.
    # To avoid overflow in exp(ratio), we compute everything in log domain first.
    ...
    # ratio = exp( log_psi_swap - log_psi_orig )
    log_ratio = (log_psi_1_swap + log_psi_2_swap) - (log_psi_1 + log_psi_2)

    # We need E[exp(log_ratio)].
    # If log_ratio is positive and large, this explodes.
    # If log_ratio is negative and large, this is zero.

    # Check max value to avoid numerical explosion
    max_val = log_ratio.max().item()
    if max_val > 50:
        # If the swapped state is wildly more probable than data state,
        # it means the model is broken (entropy will be negative).
        # We clamp it to warn but allow calculation.
        pass
...
if __name__ == "__main__":
    ...
    num_hidden = 64  # Increased capacity
    num_epochs = 150 # More epochs for PCD

    # PCD Hyperparams
    k_steps = 5      # PCD maintains chain, so small k is fine
    init_lr = 0.005  # PCD needs lower LR than CD
    ...
    # 4. INIT MODEL (Pass batch_size for persistent buffer init)
    model = RBM(num_visible=num_visible, num_hidden=num_hidden, k=k_steps, T=1.0, batch_size=batch_size)
    optimizer = torch.optim.Adam(model.parameters(), lr=init_lr)

    # 5. TRAIN
    print("Starting Training (Persistent Contrastive Divergence)...")
    ...
    # Long thermalization for final sampling
    ...
    plt.plot(l_axis_model, s2_curve_model, 'ro-', label=f'RBM (PCD) $\Delta={TARGET_DELTA}$')

    plt.title(f"Entanglement Entropy (N={CHAIN_LENGTH}, PCD Training)")
