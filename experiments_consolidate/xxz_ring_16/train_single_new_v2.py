...
class RBM(nn.Module):
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        l1_strength = aux_vars.get("l1_strength", 0.0)
        ...
        # L1 Regularization (Encourages sparsity/local correlations)
        if l1_strength > 0:
...
if __name__ == "__main__":
    ...
    # === DATA AUGMENTATION (SPIN FLIP SYMMETRY) ===
    print("Augmenting dataset with Spin-Flip Symmetry...")
    original_vals = torch.as_tensor(ds.values, dtype=torch.uint8)
    flipped_vals = 1 - original_vals # Flip 0->1, 1->0

    # Combine
    combined_vals = torch.cat([original_vals, flipped_vals], dim=0)
    # Dummy system params (needed for loader structure but unused)
    combined_params = torch.cat([torch.as_tensor(ds.system_params), torch.as_tensor(ds.system_params)], dim=0)

    # Update dataset
    ds.values = combined_vals.numpy()
    ds.system_params = combined_params.numpy()
    print(f"New dataset size: {ds.values.shape[0]}")
    # ===============================================

    # 3. SETUP
    batch_size = 512
    num_visible = CHAIN_LENGTH
    num_hidden = 64   # Alpha=4 for Capacity
    num_epochs = 100
    k_steps = 30
    init_lr = 0.05
    l1_strength = 2e-5 # Small L1 to encourage local features
    ...
    # 5. TRAIN
    print("Starting Training (SGD + CD-k + Augmentation + L1)...")
    ...
    plt.plot(l_axis_model, s2_curve_model, 'ro-', label=f'RBM (Augmented) $\Delta={TARGET_DELTA}$')
