...
class RBM(nn.Module):
    def __init__(self, num_visible: int, num_hidden: int, k: int = 1, T: float = 1.0):
        ...
        # TUNING 1: 0.05 is the sweet spot.
        # 0.01 -> Landscape too flat -> Entropy ~ 0
        # 0.10 -> Instability -> Flip Rate 0.5
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        # TUNING 2: CD-k starting from DATA (Standard)
        # This is essential for XXZ to stay in the S_z=0 sector.
...
def train_step(model: nn.Module, optimizer: torch.optim.Optimizer,
    ...
    # TUNING 3: Gradient Clipping
    # This prevents the parameters from exploding and causing the "0.5 flip rate" crash
...
def compute_renyi_entropy_from_samples(
    ...
    # Clip to avoid NaNs, but if this hits, it means Swap energy is WAY lower (very unlikely for ground states)
    log_ratio = torch.clamp(log_ratio, max=20.0, min=-50.0)
...
if __name__ == "__main__":
    ...
    # TUNING 4: High Capacity
    num_hidden = 64
    num_epochs = 150
    # TUNING 5: CD-k Steps (start small, could increase but 10 is usually fine)
    k_steps = 10
    init_lr = 0.01
    ...
    # 5. TRAIN
    print("Starting Training (CD-k, Init 0.05, ClipGrad)...")
    ...
    # Anneal temp to ensure mixing during generation
