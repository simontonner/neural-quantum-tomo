...
# Assuming this script is running in a subdirectory (e.g., /experiments/)
# and data_handling.py is in the parent.
...
class RBM(nn.Module):
    def __init__(self, num_visible: int, num_hidden: int, k: int = 1, T: float = 1.0):
        ...
        # CRITICAL: Low variance initialization (from the TFIM script)
    ...
    def _free_energy(v: torch.Tensor, W: torch.Tensor, b: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        # Free Energy F(v) = -b.v - sum_j log(1 + exp(W.v + c))
        term1 = -(v * b).sum(dim=-1)
    ...
    def log_score(self, v: torch.Tensor) -> torch.Tensor:
        """
        Returns log(psi(v)).
        Assuming psi(v) = exp(-FreeEnergy(v) / 2) -> log(psi) = -0.5 * FreeEnergy
        """
        fe = self._free_energy(v, self.W, self.b, self.c)
        return -0.5 * fe / self.T

    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        # Unpack batch from MeasurementLoader
        # tuple is (values, bases, cond) -> We IGNORE cond and bases
        values, _, _ = batch
        ...
        # 1. CD-k Sampling
        # Start from noise (better mixing than starting from data usually)
        ...
        # 2. Loss Calculation (Contrastive Divergence)
        ...
        # Optional L2 on weights (usually not needed if init is small)
        if l2_strength > 0:
            loss += l2_strength * (self.W.pow(2).sum())

        # Stats
        flip_rate = (v_data != v_model).float().mean()
...
def train(model: nn.Module, optimizer: torch.optim.Optimizer, loader, num_epochs: int,
    ...
    print(f"{'Epoch':<6} | {'Loss':<9} | {'FE Diff':<9} | {'FlipRate':<9} | {'LR':<9}")
    print("-" * 55)
...
if __name__ == "__main__":
    ...
    TARGET_DELTA = 1.00  # We only train on this
    ...
    num_hidden = 32  # Good size for N=16
    num_epochs = 10
    k_steps = 200     # CD-k
    init_lr = 1e-2
    final_lr = 1e-3
    ...
    # 5. TRAIN
    print("Starting Training (Standard RBM, No Conditioning)...")
    ...
    T_eval = 1.0
    T_schedule = torch.full((model.k,), T_eval, device=next(model.parameters()).device)
    ...
    if ref_file.exists():
        ...
        if mask.any():
            ...
            print("Reference file found, but delta 1.00 missing.")
    else:
        print("Reference file not found.")

    # Plot Model
    plt.plot(l_axis_model, s2_curve_model, 'ro-', label=f'RBM (No Cond) $\Delta={TARGET_DELTA}$')
