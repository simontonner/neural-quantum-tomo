...
if __name__ == "__main__":
    ...
    for delta in delta_values:
        ...
        pairs = [(i, (i + 1) % chain_length) for i in range(chain_length)]
        z_mag = compute_czz(samples, pairs)
        z_mags[delta] = z_mag
        print(f"  Computed z-magnetization: {z_mag:+.4f}")
    ...
    for i, delta in enumerate(delta_values):
        ...
        if delta not in results_data:
            ax.set_title(f"$\Delta={delta}$ (No Data)", fontsize=14)
            continue
        ...
        ax.set_title(f"$\Delta = {delta}$, z_mag={z_mag:+.4f}", fontsize=14)
