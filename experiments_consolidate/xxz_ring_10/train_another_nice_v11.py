...
class SymmetricRBM(nn.Module):
    ...
    def initialize_weights(self):
        # Slightly tighter init to work well with Weight Decay
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        # CD-k: We maintain k > 1 to ensure Czz remains accurate
...
if __name__ == "__main__":
    ...
    # CD-10: Enough steps to learn correlations, but not so many we overfit peaks immediately.
    CD_K = 10
    # Alpha-6: Enough capacity for the critical state complexity.
    ALPHA = 6
    # Weight Decay: The magic sauce.
    # 1e-4 prevents the weights from exploding, ensuring the wavefunction
    # retains 'width' (Entropy) rather than collapsing to a delta function.
    WEIGHT_DECAY = 1e-4

    EPOCHS = 150
    LR = 0.003  # Slightly lower LR to stabilize the weight decay interaction
    ...
    # Initialize
    print(f"Training Unified RBM (Alpha={ALPHA}, CD-{CD_K}, WD={WEIGHT_DECAY})...")
    torch.manual_seed(42)
    rng = torch.Generator().manual_seed(42)

    model = SymmetricRBM(CHAIN_LENGTH, ALPHA, CD_K)

    # CRITICAL: Include weight_decay here
    ...
    ax[0].set_title('Correlation (C_zz)')
    ...
    ax[1].plot(l_axis, s2_curve, 'r-o', label='Unified RBM')

    # Try to plot Reference if available
    ...
    ax[1].set_title('Entanglement Entropy (S2)')
