...
def compute_data_czz(loader) -> float:
    ...
    # Iterate through the entire loader once
    for batch in loader:
        values, _, _ = batch
        # Convert binary 0/1 to spins -1/1
        spins = 2.0 * values.float() - 1.0
        # Calculate neighbor correlation: <sigma_i * sigma_{i+1}>
        # mean() averages over batch AND chain length
...
def get_current_czz(model: RBM, rng: torch.Generator) -> float:
    """Quickly compute Czz on a small batch of Model samples to track progress."""
    model.eval()
...
if __name__ == "__main__":
    ...
    # === NEW: Calculate and Print Baseline Czz ===
    ...
    ax2 = ax[0].twinx()
    # Model Czz
    ax2.plot(history['czz'], label=r'Model $C_{zz}$', color='orange', linestyle='-')
    # Baseline Czz Line
