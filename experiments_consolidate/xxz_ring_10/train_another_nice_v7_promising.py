...
class SymmetricRBM(nn.Module):
    def __init__(self, num_visible: int, alpha: int, k: int):
        ...
        # Weights: [1, Alpha, N] - One kernel per hidden feature
    ...
    def _free_energy(self, v: torch.Tensor) -> torch.Tensor:
        ...
        # Broadcast hidden bias c [Alpha] across N sites
    ...
    def _gibbs_step(self, v: torch.Tensor, rng: torch.Generator):
        ...
        # --- Backward: h -> v ---
        # For circular convolution, the transpose operation is convolution
        # with the spatially flipped kernel.

        # Flip kernel: [1, Alpha, N]
    ...
    def get_full_psi(self):
        """
        Returns the Positive Wavefunction (Amplitude only).
        Matches the Marshall=True reference state.
        """
        device = next(self.parameters()).device
        N = self.num_visible
        ...
        # No sign correction (Marshall state is positive)
...
if __name__ == "__main__":
    ...
    # === HYPERPARAMETERS ===
    # CD-20 helps mixing, Alpha=4 provides capacity for critical state
    CD_K = 20
    ALPHA = 4
    EPOCHS = 150
    LR = 0.005
    ...
    # Initialize Model
    print(f"Training Symmetric RBM (Alpha={ALPHA}, CD-{CD_K})...")
    ...
    for epoch in range(EPOCHS):
        ...
        if (epoch+1) % 10 == 0:
            # Check Czz
            model.eval()
            # Generate samples to check correlation
            ...
            hist_czz.append(czz)
            print(f"Ep {epoch+1}: Loss={avg_loss:.4f} | Czz={czz:.4f} (Target {data_czz:.4f})")

    # Final Entropy Calculation
    print("\nCalculating Entropy (No Sign Correction)...")
    ...
    ax[0].set_title('Correlation Convergence')
    ...
    ax[1].plot(l_axis, s2_curve, 'r-o', label='RBM (Marshall=True Data)')
    ...
    ax[1].set_title('Entanglement Entropy')
