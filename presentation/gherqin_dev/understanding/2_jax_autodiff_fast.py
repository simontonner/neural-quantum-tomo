# ============================================================
# JAX RBM Tomography — 70-epoch training with metrics & plots
# - Groups batches by basis for stable JIT shapes
# - JITs a single train_step (static k, static is_z)
# - Caches per-basis meta (sites, combos, Uc_flat)
# - Plots Fidelity & KL and phase comparison (global-phase aligned)
# ============================================================
...
class Trainer:
    ...
    def fit(self, loader, epochs=70, k=10, log_every=5,
        ...
        for ep in range(1, epochs + 1):
            ...
            # sync at epoch end
            jnp.asarray(loss_val).block_until_ready()
...
if __name__ == "__main__":
    ...
    # -------------------------------
    # Align a single global phase, then compare phases on the high-mass support.
    space = nn_state.generate_hilbert_space()
    psi_m = nn_state.psi_complex_normalized(space).reshape(-1)
    psi_t = data.target().reshape(-1).astype(CDTYPE)

    # normalize both
    nm = jnp.linalg.norm(psi_m)
    nt = jnp.linalg.norm(psi_t)
    if float(nm) > 0.0:
        psi_m = psi_m / nm
    if float(nt) > 0.0:
        psi_t = psi_t / nt

    # align global phase via inner product (fallback: match max-amplitude basis state)
    ...
    if ep_hist and ("Fidelity" in history) and ("KL" in history):
        ...
        ax1.set_title("RBM Tomography – training metrics (JAX CD-k)")
