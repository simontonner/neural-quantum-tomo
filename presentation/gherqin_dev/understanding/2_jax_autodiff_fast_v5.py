# ============================================================
# JAX RBM Tomography — donation-safe, stabilized, grouped, JIT'd
# - Basis-grouped batches + fully JIT'd inner grads
# - SGD(momentum) with smaller LR for phase RBM (optax.multi_transform)
# - Gradient accumulation
# - Strong eps guards in rotated-overlap + Gibbs
# - Uses jax.tree_util.tree_map via alias `tmap` (no deprecation)
# - Plots: metrics and phase comparison
# - FIX: no param donation; only donate (pos, neg, rng) to avoid "buffer donated" errors
# ============================================================
...
# ---------- Apply tensor product without building it ----------
...
@partial(jax.jit, static_argnames=("k",), donate_argnums=(3,))
def _rbm_gibbs_steps(params: Dict[str, jnp.ndarray], k: int, initial_state: jnp.ndarray,
                     key: jax.Array, eps_p: float = 1e-6):
    """k-step block Gibbs from initial_state in {0,1}. Donate only key."""
    W, b, c = params["W"], params["b"], params["c"]
...
# donate only (pos_batch, neg_batch, rng); NEVER params; not basis meta
@partial(jax.jit, static_argnames=("k","is_z"), donate_argnums=(1,2,3))
...
# ---------- Trainer with momentum, per-group LR, grad accumulation ----------
class Trainer:
    ...
    def fit(self, loader, epochs=70, k=10, log_every=5,
        ...
        for ep in range(1, epochs + 1):
            ...
            for pos_batch, neg_batch, basis_tuple, is_z in loader.iter_epoch():
                ...
                # params NOT donated; only (pos, neg, rng) donated inside jit
            ...
            # force compute to finish (profiling / stable timing)
