...
class ComplexWaveFunction:
    ...
    def _positive_phase_loss(self, samples: torch.Tensor,
        ...
        total = loss_rot + loss_z
        if return_split:
            return total, loss_z, loss_rot, n_z, n_rot
        return total
    ...
    def fit(self, loader, epochs=100, k=10, lr=5e-2, log_every=5,
        ...
        if lr_milestones is None:
            lr_milestones = [(60, 1e-2), (90, 5e-3)]
        lr_milestones = sorted([(int(ep_m), float(lr_m)) for ep_m, lr_m in lr_milestones], key=lambda x: x[0])
        ...
        for ep in range(1, epochs + 1):
            for milestone_epoch, milestone_lr in lr_milestones:
                if ep == milestone_epoch:
                    _set_lr(milestone_lr)
            ...
            for pos_batch, neg_batch, bases_batch in loader.iter_epoch():
                ...
                L_pos, L_z, L_rot, n_z, n_rot = self._positive_phase_loss(
                    pos_batch, bases_batch, return_split=True
                )
                ...
                pos_sum += float(pos_term.item())
                neg_sum += float(neg_term.item())
                zpos_sum += float((L_z / max(n_z, 1)).item()) if n_z > 0 else 0.0
                rotpos_sum += float((L_rot / max(n_rot, 1)).item()) if n_rot > 0 else 0.0
            ...
            if (target is not None) and (ep % log_every == 0):
                ...
                history["PosLoss"].append(pos_sum / max(nbatches, 1))
                history["NegLoss"].append(neg_sum / max(nbatches, 1))
                history["ZPosLoss"].append(zpos_sum / max(nbatches, 1))
                history["RotPosLoss"].append(rotpos_sum / max(nbatches, 1))
                history["GradNorm"].append(grad_sum / max(nbatches, 1))
                ...
                if print_metrics:
                    print(
                        f"Epoch {ep}: Fidelity = {fid_val:.6f} | KL = {kl_val:.6f} | "
                        f"max |Δφ| = {ph_stats['max_abs_phase_err']:.6f} | "
                        f"mean |Δφ| = {ph_stats['mean_abs_phase_err']:.6f} | "
                        f"pos = {history['PosLoss'][-1]:.6f} | neg = {history['NegLoss'][-1]:.6f} | "
                        f"zpos = {history['ZPosLoss'][-1]:.6f} | rotpos = {history['RotPosLoss'][-1]:.6f} | "
                        f"grad = {history['GradNorm'][-1]:.6f} | lr = {history['LR'][-1]:.6f}"
                    )
...
if __name__ == "__main__":
    ...
    if ep_hist and ("Fidelity" in history) and ("KL" in history):
        fig, ax1 = plt.subplots(figsize=(7.0, 4.5), dpi=140)
        ax2 = ax1.twinx()
        ...
        ax1.set_title("Pooled-CD autodiff training metrics")
        ...
        fig_dbg, axd = plt.subplots(figsize=(7.0, 4.5), dpi=140)
        axd.plot(ep_hist, history["PosLoss"], marker="o", label="pos")
        axd.plot(ep_hist, history["NegLoss"], marker="s", label="neg")
        axd.plot(ep_hist, history["ZPosLoss"], marker="^", label="zpos")
        axd.plot(ep_hist, history["RotPosLoss"], marker="v", label="rotpos")
        axd.set_xlabel("Epoch")
        axd.set_ylabel("Average term value")
        axd.set_title("CD surrogate diagnostics")
        axd.grid(True, alpha=0.3)
        axd.legend(loc="best")
        fig_dbg.tight_layout()

        fig_g, axg = plt.subplots(figsize=(7.0, 4.0), dpi=140)
        axg.plot(ep_hist, history["GradNorm"], marker="o", label="grad norm")
        axg.set_xlabel("Epoch")
        axg.set_ylabel("Gradient norm")
        axg.set_title("Gradient norm diagnostic")
