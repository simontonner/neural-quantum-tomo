...
class ComplexWaveFunction:
    ...
    def fit(self, loader, epochs=150, k=10, lr=2e-2, log_every=5,
        ...
        for ep in range(1, epochs + 1):
            ...
            beta_cd = 1.0 if beta_cd_schedule is None else float(beta_cd_schedule(ep))
            ...
            for pos_batch, neg_batch, bases_batch in loader.iter_epoch():
                ...
                loss = pos_term - beta_cd * neg_term
                ...
                pos_vals_epoch.append(float(pos_term.item()))
                neg_vals_epoch.append(float(neg_term.item()))

                if cnt_z > 0:
                    zpos_vals_epoch.append(float((L_z_only / cnt_z).item()))
                if cnt_rot > 0:
                    rotpos_vals_epoch.append(float((L_rot_only / cnt_rot).item()))

                grad_vals_epoch.append(grad_norm)
            ...
            if ep % log_every == 0:
                ...
                history["Pos"].append(float(np.mean(pos_vals_epoch)))
                history["Neg"].append(float(np.mean(neg_vals_epoch)))
                history["ZPos"].append(float(np.mean(zpos_vals_epoch)) if zpos_vals_epoch else float("nan"))
                history["RotPos"].append(float(np.mean(rotpos_vals_epoch)) if rotpos_vals_epoch else float("nan"))
                history["GradNorm"].append(float(np.mean(grad_vals_epoch)))
                history["LR"].append(float(current_lr))
                history["BetaCD"].append(float(beta_cd))
...
def beta_cd_schedule(epoch: int) -> float:
    if epoch < 55:
        return 1.0
    if epoch < 85:
        return 0.5
...
if __name__ == "__main__":
    ...
    history = nn_state.fit(
        ...
        beta_cd_schedule=beta_cd_schedule,
    )
    ...
    if ep_hist and ("Fidelity" in history) and ("KL" in history):
        ...
        ax1.set_title("RBM Tomography - pooled CD autodiff (annealed CD weight)")
