...
"""
5-seed pooled-CD ComplexRBM tomography with opposite sigmoid schedules:

- amplitude LR: 0.03 -> 0.003
- phase LR:     0.005 -> 0.03

Training setup:
- batch size 512
- negative batch size 512
- k = 10
- epochs = 20
- Adam for both amplitude and phase RBMs

Outputs:
- phase comparison plot across seeds
- wrapped phase error plot across seeds
- CSV summaries
"""
...
def build_schedules(cfg, num_steps):
    s = cfg["schedule"]
    am_schedule = get_sigmoid_curve(s["am_init_lr"], s["am_final_lr"], num_steps, s["falloff"])
    ph_schedule = get_sigmoid_curve(s["ph_init_lr"], s["ph_final_lr"], num_steps, s["falloff"])
...
def train_loop(model, optimizer_am, optimizer_ph, loader, dataset, cfg):
    ...
    history = {"epoch": [], "Fidelity": [], "LR_AM": [], "LR_PH": []}
    num_steps = num_epochs * len(loader)
    am_schedule, ph_schedule = build_schedules(cfg, num_steps)
    global_step = 0
...
def run_multiseed_phase_experiment(base_cfg, seeds):
    ...
    for seed in seeds:
        ...
        seed_summaries.append({
            ...
            "final_lr_am": float(history["LR_AM"][-1]),
            "final_lr_ph": float(history["LR_PH"][-1]),
        })
...
def plot_phase_aggregate(phase_df, summary_df, out_prefix="multiseed_phase_ampdown_phup_bs512_epoch20"):
    ...
    ax1.set_title("5-seed phase comparison - bs=512, amp 0.03→0.003, phase 0.005→0.03")
    ...
    ax2.set_title("5-seed wrapped phase error - bs=512, amp 0.03→0.003, phase 0.005→0.03")
