    ...
    def fit(self, loader, epochs=70, k=10, lr=1e-1, log_every=5, optimizer=torch.optim.SGD, optimizer_args=None,
        ...
        for ep in range(1, epochs + 1):
            ...
            for pos_batch, neg_batch, bases_batch in loader.iter_epoch():
                ...
                # Because every basis has the same number of shots in this benchmark,
                # the pooled positive-batch average below is an unbiased estimator of
                # the scaled basis-averaged practical objective.
...
class TomographyDataset:
    """
    Container for flattened training samples, per-sample bases, evaluation bases,
    and target state.

    Expected old-style file structure:
        - train_path: matrix of measurement outcomes, shape (N, n)
        - train_bases_path: matrix of basis labels, shape (N, n)
        - psi_path: target state amplitudes as two columns (re, im)
    ...
    def __init__(
        ...
        self.bases = np.asarray(eval_bases_np)
        ...
        self._eval_bases_tuples = [tuple(row) for row in np.asarray(self.bases, dtype=object)]
...
class RBMTomographyLoader:
    ...
    This keeps the original working semantics:
      ...
      - basis rows remain aligned with the positive samples
    """
    ...
    def set_seed(self, seed: Optional[int]):
        ...
        Note:
            Leaving this unused preserves the original global-RNG behavior.
        """
        if seed is None:
            self._gen = None
        else:
    ...
    def iter_epoch(self):
        ...
        pos_bases_perm = [self._bases_list[i] for i in perm_idx]
