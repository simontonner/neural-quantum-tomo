# RBM wavefunction — updated to train from per-basis files under measurements/
# -----------------------------------------------------------------------------
# What changed vs your OLD TRAINING CODE:
# - New dataset/loader that ingest files like:
#     measurements/w_phase_state.txt                         (target psi: Re, Im)
#     measurements/w_phase_<BASIS>_<shots>.txt               (one line per shot)
#   where each line encodes outcomes by case: uppercase=0, lowercase=1
#   e.g. "ZzZX" means [0,1,0,0] measured in basis "ZZZX".
# - No monolithic w_state_meas.txt / w_state_basis.txt anymore.
# - Everything else (cdouble math, rotations, explicit grads, metrics) unchanged.
# -----------------------------------------------------------------------------

from math import ceil, sqrt, prod
from itertools import chain
from typing import Iterable, List, Tuple, Optional, Dict, Sequence
...
# ===============================
# DATA: Per-basis directory loader
# ===============================
class PerBasisTomographyDataset:
    """
    Load tomography data from a directory like:
      measurements/
        w_phase_state.txt                    -> target psi: two columns (Re, Im)
        w_phase_<BASIS>_<shots>.txt          -> outcomes encoded per line by case

    Encoding:
      For each line (length n) and corresponding BASIS (length n):
        char is uppercase -> 0, lowercase -> 1
        e.g. line "ZzZX" => [0,1,0,0] for basis "ZZZX"

    Exposes:
      - self.train_samples : (N, n) float64 on device
      - self.train_bases   : list[tuple[str,...]] length N (CPU)
      - self.target_state  : (2^n,) complex (cdouble) on device
      - self._unique_bases : list[tuple[str,...]] unique bases (CPU)
      - self._z_indices    : indices of rows with all-'Z' basis (CPU long)
    """
    def __init__(self,
        ...
        self.train_samples = torch.tensor(np.stack(samples, axis=0), dtype=DTYPE, device=self.device)
        ...
        expected_dim = 1 << n
        if self.target_state.numel() != expected_dim:
            raise ValueError(f"State length {self.target_state.numel()} != 2^{n} ({expected_dim}).")
    ...
    def _parse_basis_code_from_filename(name: str, prefix: str) -> str:
        """
        Accept names like 'w_phase_XXZZ_5000.txt' and return 'XXZZ'.
        """
    ...
    # API mirrored to previous dataset
    def __len__(self):
    ...
    def eval_bases(self) -> List[Tuple[str, ...]]:
        return list(self._unique_bases)
...
class RBMTomographyLoader:
    ...
    Purpose-built loader that yields (pos_batch, neg_batch, bases_batch) per epoch:
      ...
      - Negative samples are drawn with replacement from the Z-only pool.
...
if __name__ == "__main__":
    ...
    # hyperparams (unchanged defaults; adjust to taste)
    epochs = 70
    pbs = 100
    nbs = 100
    lr = 1e-1
    ...
    # Optionally give the loader an independent RNG:
    ...
    # -------------------------------
    # Phase comparison (primary diagnostic)
    # -------------------------------
    with torch.no_grad():
        space = nn_state.generate_hilbert_space()
        ...
        mass_cut = 0.99
        k_cap = 512
        k = int(min(np.searchsorted(cum, mass_cut) + 1, k_cap, len(order)))
        sel = order[:k]

        # Plot: target vs model phases (aligned)
        fig_p, axp = plt.subplots(figsize=(7.2, 3.8), dpi=150)
        axp.plot(range(k), phi_t[sel], marker=".", linestyle="", label="target phase")
        axp.plot(range(k), phi_m[sel], marker="x", linestyle="", label="model phase (aligned)")
        ...
        axe.plot(range(k), dphi[sel], marker=".", linestyle="", label="Δphase (wrapped)")
