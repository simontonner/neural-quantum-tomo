...
class RBM(nn.Module):
    def __init__(self, num_visible: int, num_hidden: int, k: int = 1, T: float = 1.0,
        ...
        self.initialize_weights(w_std=0.05) # Slightly smaller init for stability
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        reset_prob = aux_vars.get("reset_prob", 0.05)

        # === PCD WITH PARTIAL RESET ===
        # 1. Detach
        v_model = self.persistent_chain.detach().to(v_data.device)

        # 2. Partial Reset: Replace some chains with random noise to improve mixing
        batch_size = v_model.shape[0]
        n_reset = int(reset_prob * batch_size)
        if n_reset > 0:
            # Pick random indices
            indices = torch.randperm(batch_size, device=v_model.device)[:n_reset]
            # Reset them
            noise = torch.bernoulli(torch.full((n_reset, self.num_visible), 0.5, device=v_model.device))
            v_model[indices] = noise
...
def train(model: nn.Module, optimizer: torch.optim.Optimizer, loader, num_epochs: int,
    ...
    for epoch in range(num_epochs):
        ...
        for batch in loader:
            # reset_prob=0.10 means 10% of chains are randomized every step
            aux_vars = { "rng": rng, "reset_prob": 0.10 }
...
def compute_renyi_entropy_from_samples(
    ...
    # ratio = exp( log_swapped - log_orig )
    log_ratio = (log_psi_1_swap + log_psi_2_swap) - (log_psi_1 + log_psi_2)

    # Clip large positive values to prevent NaN in exp
    # If a state is 10^20 times more likely after swap, something is wrong,
    # but we clip to handle numerical spikes.
    log_ratio = torch.clamp(log_ratio, max=50.0)
...
if __name__ == "__main__":
    ...
    # Increase hidden units for complex correlations
    num_hidden = 128
    num_epochs = 150
    # Steps for PCD Gibbs
    k_steps = 10
    ...
    # ADDED WEIGHT DECAY: Keeps the energy landscape smooth
    optimizer = torch.optim.Adam(model.parameters(), lr=init_lr, weight_decay=1e-4)

    # 5. TRAIN
    print("Starting Training (PCD + Partial Reset + Weight Decay)...")
    ...
    # Annealing for evaluation sampling
    T_schedule = torch.cat([
        torch.linspace(2.0, 1.0, 50),
        torch.ones(100)
    ...
    plt.plot(l_axis_model, s2_curve_model, 'ro-', label=f'RBM (PCD+Reset) $\Delta={TARGET_DELTA}$')
