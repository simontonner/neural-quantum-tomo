...
def run_experiment(density: int, loader: MeasurementLoader, config: Dict):

    num_hidden = config['N'] * density
    print(f"\n>>> STARTING EXPERIMENT: Alpha={density} (Hidden={num_hidden}) <<<")

    SEED = 42
    torch.manual_seed(SEED)
    rng = torch.Generator().manual_seed(SEED)

    model = RBM(num_visible=config['N'], num_hidden=num_hidden, k=config['k'], T=1.0)

    # NO REGULARIZATION (Based on previous result)
...
if __name__ == "__main__":
    ...
    config = {
        'N': CHAIN_LENGTH,
        'epochs': 120,    # Slightly longer
        'lr': 0.05,       # Slightly lower LR for stability without L2
        'k': 20           # Increased CD-k for better mixing
    }
    ...
    # EXPERIMENT: Sweep Hidden Unit Density
    densities = [1, 2, 4] # Alpha

    results = {}
    curves = {}

    for alpha in densities:
        model, duration, czz = run_experiment(alpha, loader, config)

        print(f"Evaluating S2 for Alpha={alpha}...")
        ...
        results[alpha] = {'time': duration, 'czz': czz}
        curves[alpha] = s2_curve

    # REPORT
    print("\n" + "="*50)
    print("FINAL COMPARISON REPORT (CAPACITY SWEEP)")
    print("="*50)
    print(f"{'Alpha':<6} | {'Hidden':<8} | {'Final Czz':<10} | {'S2(L=8)':<10}")
    print("-" * 50)

    for alpha in densities:
        res = results[alpha]
        s2_end = curves[alpha][-1]
        print(f"{alpha:<6} | {alpha*CHAIN_LENGTH:<8} | {res['czz']:<10.4f} | {s2_end:<10.4f}")
    ...
    colors = {1: "blue", 2: "orange", 4: "red"}

    for alpha in densities:
        plt.plot(l_axis, curves[alpha], color=colors[alpha], marker="o",
                 label=f"Alpha={alpha} (H={alpha*CHAIN_LENGTH})")

    plt.title(f"Capacity Comparison (XXZ N={CHAIN_LENGTH}, $\Delta=1.0$)")
