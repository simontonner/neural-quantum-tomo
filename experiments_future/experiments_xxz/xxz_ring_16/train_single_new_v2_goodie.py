...
class RBM(nn.Module):
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        # Note: We compute loss such that gradients match CD
...
def compute_nearest_neighbor_corr(samples: torch.Tensor) -> float:
    ...
    Input 0/1 -> Mapped to -1/+1 for correlation."""
    # Map 0 -> -1, 1 -> 1
    spins = 2.0 * samples.float() - 1.0

    # Periodic boundary conditions usually for these datasets,
    # but let's check simple linear NN for now.
    # N-1 pairs
...
def train(model: nn.Module, optimizer: torch.optim.Optimizer, loader, num_epochs: int,
    ...
    for epoch in range(num_epochs):
        ...
        # XXZ Delta=1.0 Ground state has strong antiferromagnetic correlations (negative C_zz)
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # 3. SETUP
    batch_size = 512  # Smaller batch size for SGD
    num_visible = CHAIN_LENGTH
    num_hidden = 32   # Matching Paper (alpha=1 for N=20 -> ~16 for N=16)
    num_epochs = 500
    k_steps = 20      # CD-10
    init_lr = 0.1     # High LR for SGD (paper suggests 0.01 - 0.1 range for SGD usually)
    ...
    plt.plot(l_axis_model, s2_curve_model, 'ro-', label=f'RBM (SGD) $\Delta={TARGET_DELTA}$')

    plt.title(f"Entanglement Entropy (N={CHAIN_LENGTH}, SGD)")
