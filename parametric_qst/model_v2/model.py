...
class ConditionalRBM(nn.Module):
    ...
    def forward(self, batch: Tuple[torch.Tensor, ...], aux_vars: Dict[str, Any]):
        ...
        # starting the s chain with ones ensures that we don't flip our CD chain immediately away from the data
        # let's try to form one well first before trying to mirror it
        s0 = torch.ones((B, 1), device=v_model.device, dtype=v_model.dtype)
