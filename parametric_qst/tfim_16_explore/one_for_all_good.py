...
# --- OPTIMAL SAMPLING PARAMETERS (Derived from your Tuning) ---
ANNEAL_T_START = 5.0
ANNEAL_STEPS = 1000
TOTAL_SAMPLES = 50_000   # Sufficient for good statistics
CHUNK_SIZE = 10_000
...
class ConditionalRBM(nn.Module):
    ...
    # --- Annealed Generation ---
    @torch.no_grad()
    def generate_annealed(self, cond: torch.Tensor, steps: int, t_start: float, rng: torch.Generator) -> torch.Tensor:
        ...
        for i in range(steps):
            ...
            self.T = t_start + (1.0 - t_start) * frac # Cool: t_start -> 1.0
...
model_dtype = next(model.parameters()).dtype  # <--- FIXED: Get Model Dtype
...
print(f"\n=== Starting Entropy Scan ===")
print(f"Annealing: T={ANNEAL_T_START} -> 1.0 | Steps={ANNEAL_STEPS} | Samples={TOTAL_SAMPLES}")

for i, h_val in enumerate(all_h):
    pt_type = "support" if any(np.isclose(h_val, s, atol=1e-3) for s in h_support) else "interpolated"

    # 1. Generate with Annealing (Using explicit model dtype)
    cond = torch.tensor([[h_val]], device=DEVICE, dtype=model_dtype).expand(TOTAL_SAMPLES, -1)
    with torch.no_grad():
        samples = model.generate_annealed(cond, ANNEAL_STEPS, ANNEAL_T_START, rng)

    # 2. Score (Using explicit model dtype)
...
ax2d.set_title("Renyi Entropy Cuts")
