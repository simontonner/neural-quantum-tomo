...
class ConditionalRBM(nn.Module):
    def __init__(self, num_visible: int, num_hidden: int, cond_dim: int,
                 ...
                 init_std: float = 0.01): # <--- Added init_std parameter
        super().__init__()
        self.num_visible = num_visible
        self.num_hidden = num_hidden
        self.k = k
    ...
    def initialize_weights(self):
        # Critical for RBMs: Initial weight scale
        nn.init.normal_(self.W, std=self.init_std)
...
def train_one_trial(config, train_ds, h_eval_list, all_states, state_dir, epochs=50):
    ...
    # Use CPU generator for DataLoader shuffle reproducibility within trial
    rng_loader = torch.Generator().manual_seed(42)
    loader = MeasurementLoader(train_ds, batch_size=batch_size, shuffle=True, drop_last=False, rng=rng_loader)

    model = ConditionalRBM(
        num_visible=16, num_hidden=n_hidden, cond_dim=1,
        conditioner_width=cond_width, k=cd_k, init_std=init_std
    ).to(device)

    optimizer = torch.optim.Adam(model.parameters(), lr=lr_init)

    total_steps = epochs * len(loader)
    schedule_fn = get_sigmoid_curve(lr_init, lr_final, total_steps, falloff=0.005)
...
def run_sweep():
    ...
    SYSTEM_SIZE = 16
    FILE_SUFFIX = "5000000"
    SAMPLES_PER_FILE = 10_000 # Keep this small for speed

    h_support = [0.50, 0.80, 0.95, 1.00, 1.05, 1.20, 1.50]
    # Evaluate on the hardest points (Novel points + Critical point)
    h_eval   = [0.60, 0.70, 1.00, 1.30, 1.40]
    ...
    header = f"{'#':<3}|{'Btch':<4}|{'Hid':<3}|{'LR_0':<8}|{'Std':<5}|{'Nse':<4}|{'k':<2}|{'Time(s)':<7}|{'MinOv':<7}"
    print(header)
    print("-" * len(header))

    for i in range(NUM_TRIALS):
        # Extended search space based on previous findings
        config = {
            'batch_size': int(np.random.choice([1024, 2048, 4096])),
            'n_hidden': int(np.random.choice([64, 80, 128])),
            'cond_width': 64, # Fix this, it's less critical
            # Previous best was ~0.012, so we push range higher [0.008, 0.035]
            'lr_init': 10 ** np.random.uniform(-2.1, -1.45),
            # We vary init_std now. Standard is 0.01. Maybe 0.05 helps?
            'init_std': np.random.choice([0.01, 0.03, 0.08]),
            'cd_k': int(np.random.choice([5, 10, 15, 20])),
            'noise_frac': np.random.choice([0.02, 0.04, 0.06, 0.08])
        }

        start_t = time.time()
        min_ov, avg_ov, ov_dict, state_dict = train_one_trial(
            config, ds, h_eval, all_states, state_dir, epochs=EPOCHS_PER_TRIAL
        )
        dur = time.time() - start_t

        trials.append({**config, 'min_ov': min_ov, 'avg_ov': avg_ov, 'duration': dur})

        print(f"{i+1:<3}|{config['batch_size']:<4}|{config['n_hidden']:<3}|{config['lr_init']:.1e} |{config['init_std']:<5}|{config['noise_frac']:<4}|{config['cd_k']:<2}|{dur:<7.1f}|{min_ov:.5f}")
    ...
    # --- RESULTS ---
    print("\n" + "="*40)
    print("RESULTS (Sorted by Min Overlap)")
    print("="*40)

    df = pd.DataFrame(trials)
    # Calculate an 'efficiency' score? No, just look at the table.
    df = df.sort_values(by="min_ov", ascending=False)

    cols = ['min_ov', 'avg_ov', 'duration', 'batch_size', 'n_hidden', 'lr_init', 'init_std', 'noise_frac', 'cd_k']
    print(df[cols].head(6))

    best = df.iloc[0]
    print(f"\nBest Config found (Min Ov: {best['min_ov']:.5f}):")
    print(best.to_dict())
