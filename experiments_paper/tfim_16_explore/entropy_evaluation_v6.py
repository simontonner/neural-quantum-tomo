...
# --- 2. MODEL DEFINITION (Compatible with Training) ---
...
class ConditionalRBM(nn.Module):
    ...
    def _free_energy(self, v: torch.Tensor, b_mod: torch.Tensor, c_mod: torch.Tensor) -> torch.Tensor:
        """
        SYMMETRIZED Free Energy (Matches Training).
        P(v) is forced to be equal to P(1-v).
        """
    ...
    def log_score(self, v: torch.Tensor, cond: torch.Tensor) -> torch.Tensor:
        # --- SCALE FIX ---
        # Returns log(Psi) = 0.5 * log(P) = -0.5 * FE / T
...
def compute_renyi_entropy(samples: torch.Tensor, subs_size: int,
    ...
    # Ratio of Wavefunction Amplitudes
...
print(f"Applying Manual Symmetrization during generation.")
...
for i, h_val in enumerate(all_h_values):
    ...
    # B. MANUAL SYMMETRIZATION (Lazy Sampler Fix)
    # This prevents entropy from collapsing to 0 in the ferromagnetic phase
    flip_mask = (torch.rand(samples.shape[0], 1, device=device) < 0.5).float()
    samples = samples * (1 - flip_mask) + (1 - samples) * flip_mask

    # C. Score (Model class handles the 0.5 factor now)
