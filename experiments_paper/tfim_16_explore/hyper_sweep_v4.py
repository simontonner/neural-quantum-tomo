...
def train_trial(config, train_ds, h_eval, all_states):
    ...
    # Model
    model = ConditionalRBM(16, n_h, 1, k=k, init_std=0.01).to(device) # Fixed init_std to 0.01 (winner)
...
def run_refined_sweep():
    # Load slightly more data to be sure
    SAMPLES = 20_000
    h_supp = [0.50, 0.80, 0.95, 1.00, 1.05, 1.20, 1.50]
    h_eval = [0.60, 0.70, 1.00, 1.30, 1.40] # Critical points
    ...
    print(f"Refinement Dataset: {len(ds)} samples.")

    all_states = generate_all_states(16)

    # --- FOCUSED GRID ---
    # We create a specific list of high-potential candidates rather than pure random
    # Variances based on Trial 13 (Winner)

    grid = []

    # Candidate 1: The "Safe Bet" (Winner config approx)
    grid.append({'batch_size': 1024, 'n_hidden': 80, 'lr_init': 0.03, 'cd_k': 14, 'noise_frac': 0.02})

    # Candidate 2: Higher Capacity
    grid.append({'batch_size': 1024, 'n_hidden': 112, 'lr_init': 0.03, 'cd_k': 14, 'noise_frac': 0.02})

    # Candidate 3: Faster Steps (k=10)
    grid.append({'batch_size': 1024, 'n_hidden': 80, 'lr_init': 0.035, 'cd_k': 10, 'noise_frac': 0.04})

    # Candidate 4: Smaller Batches (512) -> Needs lower LR usually, or same?
    grid.append({'batch_size': 512, 'n_hidden': 80, 'lr_init': 0.02, 'cd_k': 12, 'noise_frac': 0.02})

    # Candidate 5: High Power
    grid.append({'batch_size': 1024, 'n_hidden': 128, 'lr_init': 0.03, 'cd_k': 12, 'noise_frac': 0.02})

    # Add 5 random permutations in the "High Energy" zone
    for _ in range(5):
        grid.append({
            'batch_size': int(np.random.choice([512, 1024])),
            'n_hidden': int(np.random.choice([80, 100, 128])),
            'lr_init': 10**np.random.uniform(-1.6, -1.4), # 0.025 to 0.04
            'cd_k': int(np.random.choice([10, 12, 14])),
            'noise_frac': np.random.choice([0.02, 0.04])
        })

    print(f"{'#':<2}|{'Btch':<4}|{'Hid':<3}|{'LR':<6}|{'k':<2}|{'Nse':<4}|{'Time':<5}|{'MinOv':<7}")
    print("-" * 50)
    ...
    for i, conf in enumerate(grid):
        ...
        print(f"{i+1:<2}|{conf['batch_size']:<4}|{conf['n_hidden']:<3}|{conf['lr_init']:.3f} |{conf['cd_k']:<2}|{conf['noise_frac']:<4}|{dur:<5.0f}|{min_ov:.5f}")

        if min_ov > best_ov:
            best_ov = min_ov
            torch.save(state, "best_crbm_model.pt")
    ...
    print(df[['batch_size','n_hidden','lr_init','cd_k','time','min_ov']].head(3))
