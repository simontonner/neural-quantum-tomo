...
class ConditionalRBM(nn.Module):
    ...
    def generate(self, cond: torch.Tensor, steps: int, rng: torch.Generator) -> torch.Tensor:
        ...
        b_mod, c_mod = self._compute_effective_biases(cond)
        B = cond.shape[0]
        # Annealing T=2.0 -> T=1.0 for better mixing
...
# --- 3. CORE LOGIC: BLOCK AVERAGING + LOGSUMEXP ---

def compute_renyi_stats_block_averaging(samples: torch.Tensor, subs_size: int,
                                        log_score_fn: callable, n_blocks: int = 10) -> tuple:
    """
    1. Splits the massive sample batch into 'n_blocks'.
    2. Calculates S2 for each block using stable LogSumExp.
    3. Returns Mean and Standard Error across blocks.
    """
    ...
    # Pre-calculate scores for ALL samples to avoid redundant forward passes
    ...
    for i in range(n_blocks):
        ...
        # Swap Test Logic
        ref_1_score = block_scores[:half]
        ref_2_score = block_scores[half:]

        ref_1 = block_samples[:half]
        ref_2 = block_samples[half:]
        ...
        # log_ratio = log(Psi_swapped^2 / Psi_ref^2)
        log_ratios = (swap_1_score + swap_2_score) - (ref_1_score + ref_2_score)

        # --- Stable S2 Calculation ---
        # S2 = -ln( <exp(log_ratios)> )
        ...
        s2_block = -log_mean.item()
        s2_values.append(s2_block)

    # Statistics across blocks
    s2_values = np.array(s2_values)
    mean_s2 = np.mean(s2_values)

    # Standard Error = Std / sqrt(N_blocks)
    # This represents how much the mean would wiggle if we ran the whole experiment again
    std_err = np.std(s2_values, ddof=1) / np.sqrt(n_blocks)

    return mean_s2, std_err
...
try:
    ...
    print("Model not found. Please check filename.")
    sys.exit(1)
...
# === THE MONTE CARLO UPGRADE ===
# We use ONE massive batch.
# We split it into blocks to measure the noise (error bars).
TOTAL_SAMPLES = 60_000  # High count for stability at l=6,7,8
BLOCKS = 10             # Number of statistical blocks
EVAL_STEPS = 200

results_list = []
l_axis = list(range(1, CHAIN_LENGTH // 2 + 1))
rng_eval = torch.Generator(device=device).manual_seed(999)
model_dtype = next(model.parameters()).dtype

print(f"=== GENERATING ENTROPY DATA ===")
print(f"Strategy: {TOTAL_SAMPLES} samples per point, Block Averaging ({BLOCKS} blocks).")
...
for i, h_val in enumerate(all_h_values):
    ...
    for l in l_axis:
        # returns (mean, stderr)
        s2_mean, s2_err = compute_renyi_stats_block_averaging(samples, l, scorer, n_blocks=BLOCKS)
...
for i, h in enumerate(support_h_keys):
    ...
    err_vals = subset['s2_err'].values # We have errors now!
